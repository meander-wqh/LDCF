...
public:
	...
	int num;
	int level;
	LinkTree(size_t single_table_length, size_t fingerprint_size, double single_capacity, size_t curlevel){
	...
	bool release(CuckooFilter* cf_release){
		CuckooFilter* emptyCF = new CuckooFilter(0, 0, 0, cf_release->level);
		...
		emptyCF->_0_child = cf_release->_0_child;
		emptyCF->_1_child = cf_release->_1_child;
		cf_release = NULL;
		return true;
	}
};
